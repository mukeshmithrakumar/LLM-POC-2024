...
logging.basicConfig(
    level=logging.INFO,
    handlers=[logging.FileHandler("logs/generate.log"), logging.StreamHandler(sys.stdout)],
)

# --------------------------- Set up Configurations -------------------------- #
default_config = read_configurations(default_config_path="configs/default_generate_configs.yaml")
...
ptddtype = {'float32': torch.float32, 'bfloat16': torch.bfloat16, 'float16': torch.float16}[dtype]
ctx = nullcontext() if device_type == 'cpu' else torch.amp.autocast(device_type=device_type, dtype=ptddtype)
...
load_meta = False
if (config.init_from == 'resume') and ('config' in checkpoint) and (hasattr(checkpoint['config'], 'dataset')):
