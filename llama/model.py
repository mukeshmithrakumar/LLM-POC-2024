...
def precompute_freqs_cis(dim: int, end: int, theta: float = 10000.0):
    ...
    return freqs_cis.to(TrainingConfig.device)
...
class TransformerBlock(nn.Module):
    def __init__(self, layer_id: int, args: TrainingConfig) -> None:
        super().__init__()
        self.n_heads = args.n_heads
        self.dim = args.dim
    ...
    def forward(
        ...
        h = x + F.dropout(
            self.attention(self.attention_norm(x), freqs_cis, mask, start_pos),
            p=self.dropout_rate,
            training=training,
        )
        out = h + F.dropout(
            self.feed_forward(self.ffn_norm(h)), p=self.dropout_rate, training=training
        )
        return out
...
class Llama3(nn.Module):
    ...
    def forward(
        ...
        loss = self.criterion(
            logits.view(bsz * seqlen, self.vocab_size), targets.reshape(bsz * seqlen)
        )
        return logits, loss
    ...
    def Sampler(
        ...
        ### Calculating top-p
        # creates same-size tensor of cumulative probabilities instead of individual probs
    ...
    def generate(
        ...
        tokens = (
            tokens.unsqueeze(0) if len(tokens.shape) == 1 else tokens
        ...
        for i in range(max_gen_len):
            ...
            next_token = self.Sampler(
                logits=logits, temperature=temperature, top_p=top_p, top_k=top_k
            )
